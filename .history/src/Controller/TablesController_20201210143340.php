...
class TablesController extends AbstractController
{
     ...
     */
    public function index(Request $request,TablesRepository $repo, Restaurants $restaurant,EntityManagerInterface $em,QrCode $qrCode): Response
    {
        ...
        $form->handleRequest($request);
            ...
            for ($i=1; $i<= $tablesAjouter ; $i++) { 
                ...
                $Ntable= $tables->setNumero($id+1);
                 $qrCode = new QrCode();
                 $qrCode->setText("https://127.0.0.1:8000/restaurant/$restaurant/table/$Ntable");
                 $qrCode->setSize(300);
                 $qrCode->setMargin(10); 
                 ...
                 $qrCode->setForegroundColor(['r' => 0, 'g' => 0, 'b' => 0, 'a' => 0]);
                    ...
                    $qrCode->setLabel('Scan the code', 16, __DIR__.'/../assets/fonts/noto_sans.otf');
                    $qrCode->setLogoPath("public/build/images/codeqr/$Ntable.png");
                    $qrCode->setLogoSize(150, 200);
                    $qrCode->setValidateResult(false);
                  $qr=header('Content-Type: '.$qrCode->getContentType());
