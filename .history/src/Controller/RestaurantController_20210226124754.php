...
class RestaurantController extends AbstractController
{
     ...
     */
     ...
     public function slug($restaurant,$slug,$target){


        if($restaurant->getSlug() !== $slug) {
            $this->redirectToRoute('$target', [
                'id' => $restaurant->getId(),
                'slug' => $slug
            ]);
        }
    ...
    /**
     * @Route("/restaurant/{id<[0-9]+>}-{slug}/edit", name="app_edit_restaurant", methods={"POST","GET"})
     */
    public function editRestaurant(Restaurants $restaurant,Request $request,EntityManagerInterface $em,$slug): Response
    {   
        ...
        $this->slug($restaurant,$slug,"app_edit_restaurant");
     ...
     /**
     * @Route("/restaurant/{id<[0-9]+>}-{slug}/gestion", name="app_gestion_restaurant", methods={"POST","GET"})
     */
    public function gererRestaurant(Restaurants $restaurant,Request $request,EntityManagerInterface $em,$slug): Response
    {   
        if ($restaurant->getProprietaire() != $this->getUser()) {
            $this->addFlash('danger','Accès réfusé');
           return $this->redirectToRoute('app_restaurant');
        ...
        $this->slug($restaurant,$slug,"app_gestion_restaurant");
     ...
     */
    public function activation(Request $request,Restaurants $restaurant,EntityManagerInterface $em) : Response
    {
