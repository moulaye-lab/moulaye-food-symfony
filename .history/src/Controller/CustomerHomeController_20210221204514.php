...
{
    /**
     * @Route("/restaurant/{restaurant<[0-9]+>}/table/{id}", name="customer_home")
     */
    public function index(RestaurantsRepository $repo,Restaurants $restaurant,TablesRepository $tableRepo,Request $request): Response
    {
            $resto=$repo->findByResto($restaurant);

          
            $table=$request->query->get('id');
            dd($table);
