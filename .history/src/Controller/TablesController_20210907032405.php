...
/**
    ...
    {   
        ...
        $html = $this->renderView('tables/uploads.html.twig', [
            ...
            'etatDeReservation'  => $table->getReserved()
     ...
     */
    public function reservation(Tables $table,EntityManagerInterface $em) : Response {
     ...
     $reservation= $table->getReserved();
       ...
       if ($reservation==false) {
           $tableId=$table->getId();
           $codeReserv=$tableId.rand(5,10000);
           $hashCodeReserv= password_hash($codeReserv,PASSWORD_BCRYPT , ['cost' => 10]);
            $table->setReserved(1);
            $table->setCodeReservation($codeReserv);
            $em->persist($table);
            $em->flush();


            $this->addFlash("success","la table apparaitra comme reservée/Le code de reservation est : $codeReserv à communiquer au client" );

       }else if ($reservation==true) {
          
        $table->setReserved(0);
        $table->setCodeReservation(null);
        $em->persist($table);
