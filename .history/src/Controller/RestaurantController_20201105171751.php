...
class RestaurantController extends AbstractController
{
     ...
     */
    public function CreateRestaurant(User $proprietaire,Request $request,EntityManagerInterface $em): Response
    {   
        $proprietaire=$this->getUser()->getId();
        $restaurant=new Restaurants;
        $form=$this->createForm(CreateRestaurantType::class,$restaurant);
        $form->handleRequest($request);
        
        if ($form->isSubmitted() && $form->isValid()) { 
            $restaurant->setProprietaire(User $proprietaire);
