...
class CustomerHomeController extends AbstractController
{
     ...
     */
    public function index(RestaurantsRepository $repo,Restaurants $restaurant,TablesRepository $tableRepo,$table,Request $request): Response
    {
            $resto=$repo->findByResto($restaurant);

         $tables=$tableRepo->findOneByNumeroAndResto($table,$restaurant);

         $codeReser = new CodeReservation;
         $form=$this->createForm(CodeReservationType::class,$codeReser);
